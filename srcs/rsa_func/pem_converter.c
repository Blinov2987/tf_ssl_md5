...
/*   Updated: 2021/03/27 17:15:54 by gemerald         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		get_pem_priv_form(t_rsa_args *args, t_rsa_output *out)
{
	...
	if (args->des)
	{
		...
		des_hiding(args, out);
		ft_lstadd_back(&out->pem, ft_lstnew(out->der->next->content,
				out->der->next->content_size));
	}
	ft_lstadd_back(&out->pem, base64_enc(out->der->content,
			out->der->content_size));
