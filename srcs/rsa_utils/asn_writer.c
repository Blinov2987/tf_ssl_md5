...
/*   Updated: 2021/03/19 18:12:20 by gemerald         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
size_t get_sequence_size(t_asn asn[])
{
	size_t size;
	int i = -1;

	size = 2;
	while (++i < 9 && asn[i].size)
	{
		size += asn[i].size;
		size += 2;
	}
...
t_list *collect_mem_to_output(t_asn asn[])
{
	t_list *mem;
	size_t size;
	uint8_t *mem_ptr;
	...
	asn_i = 0;
	size = get_sequence_size(asn);
	mem = (t_list *)ft_safe_memalloc(sizeof(t_list), "collect_mem");
	mem->content_size = size;
	mem->content = (uint8_t *)ft_safe_memalloc(sizeof(uint8_t), "collect_mem");
...
t_list *get_der_form(t_rsa_key key)
{
	...
	return (collect_mem_to_output(asn));
}